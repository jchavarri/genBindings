...
function runTest(dir) {
	...
	var jsOutput = babel.transformFileSync(dir.path + "/input.js", {
		plugins: [[pluginPath, { output: postCallback }]],
		parserOpts: {
			plugins: ["flow"]
		}
	});

	var expected = fs.readFileSync(dir.path + "/output.re", "utf-8");
	...
	diff
		.diffLines(normalizeLines(output), normalizeLines(expected))
